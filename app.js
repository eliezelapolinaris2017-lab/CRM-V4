...
function normalizeDB(db){
  ...
  db.visits = db.visits.map(v=>({
    ...
    type: String(v.type || guessType(v.service) || "Mantenimiento"),
    ...
    nextDate: v.nextDate || inferNextDate(v),
...
function guessType(service=""){const s=String(service).toLowerCase(); if(s.includes("cot"))return"Cotización"; if(s.includes("instal"))return"Instalación"; if(s.includes("diag"))return"Diagnóstico"; if(s.includes("repar"))return"Reparación"; if(s.includes("cobro"))return"Cobro"; return"Mantenimiento";}
function inferNextDate(v){const type = String(v.type || guessType(v.service)); if(type==="Mantenimiento") return addDaysISO(v.date || todayISO(), FOLLOWUP_DAYS); if(type==="Instalación") return addDaysISO(v.date || todayISO(), 30); if(type==="Diagnóstico" || type==="Cotización") return addDaysISO(v.date || todayISO(), 3); return "";}
function isValidISODate(v){return /^\d{4}-\d{2}-\d{2}$/.test(String(v||"")) && !Number.isNaN(new Date(`${v}T00:00:00`).getTime());}
function visitSortValue(v){return String(v?.updatedAt || v?.createdAt || v?.date || "");}
function getClientNextDateFromVisits(items=[]){
  const withNext = items
    .filter(v=>isValidISODate(v.nextDate))
    .sort((a,b)=>visitSortValue(b).localeCompare(visitSortValue(a)) || String(b.date||"").localeCompare(String(a.date||"")));
  return withNext[0]?.nextDate || "";
}
function buildIndexes(db){
  ...
  visitsByClient.forEach((items,cid)=>{
    ...
    const next=getClientNextDateFromVisits(items);
...
function getFollowups(){
  ...
  state.db.clients.forEach(client=>{
    const stats=clientTotals(client.id);
    const visits=state.indexes.visitsByClient.get(client.id)||[];
    const noVisitDays=stats.last ? daysBetween(stats.last) : 9999;
    const nextDate=stats.next;
    const isDueByNext=!!nextDate && nextDate<=today;
    const isDueByAge=!nextDate && (noVisitDays>=FOLLOWUP_DAYS || visits.length===0);

    if(client.status!=="Pausado" && (isDueByNext || isDueByAge)){
      const reason = visits.length ? (nextDate ? `Próxima vencida: ${nextDate}` : `${noVisitDays} días sin visita`) : "Sin visitas";
      rows.push({kind:"due",priority:(noVisitDays>150||isDueByNext)?"high":"med",client,lastDate:stats.last||"—",reason,action:"WhatsApp"});
    }

    visits.filter(v=>v.paymentStatus==="Pendiente"||v.paymentStatus==="Parcial"||v.outcome==="Requiere cobro")
      .forEach(v=>rows.push({kind:"payment",priority:"high",client,visit:v,lastDate:v.date,reason:`${v.paymentStatus} · ${money(v.amount)}`,action:"Cobro"}));

    visits.filter(v=>v.paymentStatus==="Cotizado"||v.outcome==="Requiere cotización"||v.type==="Cotización")
      .forEach(v=>rows.push({kind:"quote",priority:daysBetween(v.date)>7?"high":"med",client,visit:v,lastDate:v.date,reason:`${v.service}`,action:"Seguimiento"}));

    if(client.status==="VIP" && nextDate && nextDate>=today && nextDate<=addDaysISO(today,14)){
      rows.push({kind:"vip",priority:"med",client,lastDate:stats.last||"—",reason:`Próxima: ${nextDate}`,action:"Coordinar"});
    }
  });
  const seen=new Set();
  const rank={high:0,med:1,low:2};
  return rows
    .filter(r=>{const key=[r.kind,r.client.id,r.visit?.id||r.reason].join("|"); if(seen.has(key))return false; seen.add(key); return true;})
    .sort((a,b)=>rank[a.priority]-rank[b.priority] || String(a.lastDate).localeCompare(String(b.lastDate)));
}
...
function renderWhatsappActions(client){const box=$("whatsappActions"); if(!box)return; const phone=cleanPhone(client.contact); const actions=[['mantenimiento','Recordar mantenimiento'],['cotizacion','Cotización pendiente'],['cobro','Cobro pendiente'],['visita','Confirmar visita']]; box.innerHTML=actions.map(([kind,label])=>`<button class="whatsBtn" data-wa="${kind}" ${phone?'':'disabled'} type="button">${label}</button>`).join(""); box.querySelectorAll("[data-wa]").forEach(btn=>btn.addEventListener("click",()=>openWhatsApp(client,btn.dataset.wa)));}
function cleanPhone(v){const d=String(v||"").replace(/\D/g,""); if(d.length===10)return `1${d}`; if(d.length===11)return d; return d.length>=8?d:"";}
function waMessage(client,kind){const name=(client.name||"").split(" ")[0]||""; const link="https://confirmafy.com/oasis-services-pr"; const msgs={mantenimiento:`Saludos ${name}, le habla Oasis Services PR. Según nuestro historial, su unidad ya está próxima para mantenimiento. Puede agendar aquí: ${link}`,cotizacion:`Saludos ${name}, estamos dando seguimiento a la cotización pendiente. Podemos revisar disponibilidad y cerrar la fecha si desea continuar.`,cobro:`Saludos ${name}, aparece un balance pendiente relacionado al servicio realizado. Favor confirmar método de pago para cerrar el expediente.`,visita:`Saludos ${name}, le escribo para confirmar o coordinar su próxima visita de servicio con Oasis Services PR.`}; return msgs[kind]||msgs.mantenimiento;}
...
function renderFollowups(){const filter=safeVal("followupFilter")||"all"; const rows=getFollowups().filter(f=>filter==="all"||f.kind===filter); setText("fuDue",getFollowups().filter(f=>f.kind==="due").length); setText("fuQuotes",getFollowups().filter(f=>f.kind==="quote").length); setText("fuPayments",getFollowups().filter(f=>f.kind==="payment").length); const body=$("followupsBody"); body.innerHTML=rows.length?"":`<tr><td colspan="6" class="muted">Sin seguimientos pendientes.</td></tr>`; rows.forEach(f=>{const tr=document.createElement("tr"); tr.innerHTML=`<td>${priorityPill(f.priority)}</td><td><strong>${escapeHtml(f.client.name)}</strong><span class="mutedLine">${escapeHtml(f.client.contact||"")}</span></td><td>${escapeHtml(f.lastDate||"—")}</td><td>${escapeHtml(f.reason)}</td><td>${escapeHtml(f.action)}</td><td><div class="aBtns"><button class="aBtn" data-open="${f.client.id}" type="button">Abrir</button><button class="aBtn" data-waid="${f.client.id}" data-kind="${f.kind}" type="button">WhatsApp</button></div></td>`; body.appendChild(tr);}); body.querySelectorAll("[data-open]").forEach(btn=>btn.addEventListener("click",()=>{setView("clients");openProfile(btn.dataset.open);})); body.querySelectorAll("[data-waid]").forEach(btn=>btn.addEventListener("click",()=>{const c=state.indexes.clientsById.get(btn.dataset.waid); openWhatsApp(c, btn.dataset.kind==="payment"?'cobro':btn.dataset.kind==="quote"?'cotizacion':'mantenimiento');}));}
...
async function deleteClient(id){if(!id||!confirm("¿Borrar cliente y su historial?"))return; state.db.clients=state.db.clients.filter(c=>c.id!==id); const visitIds=state.db.visits.filter(v=>v.clientId===id).map(v=>v.id); state.db.visits=state.db.visits.filter(v=>v.clientId!==id); await idbDelete("clients",id); for(const vid of visitIds)await idbDelete("visits",vid); state.indexes=buildIndexes(state.db); scheduleDebouncedSync("local-change"); if(state.activeClientId===id)closeProfile(); refreshAll();}
async function saveVisit(){const clientId=safeVal("vClient"); if(!clientId)return alert("Selecciona un cliente."); const amount=Number(safeVal("vAmount")||0); if(Number.isNaN(amount))return alert("Monto inválido."); const now=isoNow(); const payload={clientId,date:safeVal("vDate")||todayISO(),type:safeVal("vType")||"Mantenimiento",service:safeVal("vService").trim()||"Servicio",amount,paymentStatus:safeVal("vPaymentStatus")||"Pagado",equipment:safeVal("vEquipment").trim(),model:safeVal("vModel").trim(),technician:safeVal("vTechnician").trim(),outcome:safeVal("vOutcome")||"Completado",nextDate:isValidISODate(safeVal("vNextDate"))?safeVal("vNextDate"):"",followedUp:safeVal("vFollowedUp")||"no",note:safeVal("vNote").trim(),updatedAt:now}; if(!payload.nextDate)payload.nextDate=inferNextDate(payload); if(state.editingVisitId){const v=state.db.visits.find(x=>x.id===state.editingVisitId); if(!v)return; Object.assign(v,payload);}else{state.db.visits.unshift({id:uid("v"),...payload,createdAt:now});} const c=state.db.clients.find(x=>x.id===clientId); if(c)c.updatedAt=now; await persistState(); closeVisitModal(); refreshAll(); openProfile(clientId);}
async function deleteVisit(id){if(!confirm("¿Borrar visita?"))return; state.db.visits=state.db.visits.filter(v=>v.id!==id); await idbDelete("visits",id); state.indexes=buildIndexes(state.db); scheduleDebouncedSync("local-change"); refreshAll(); if(state.activeClientId)openProfile(state.activeClientId);}
...
async function resetAll(){if(!confirm("¿Borrar todo local?"))return; state.db={clients:[],visits:[]}; await idbClearAll(); state.indexes=buildIndexes(state.db); closeProfile(); refreshAll();}
...
async function pullFirebaseToLocal(){const user=fbUser(); if(!user||!requireOwner(user))throw new Error("Cuenta no autorizada."); const uidVal=user.uid; const [cs,vs]=await Promise.all([clientCol(uidVal).get(),visitCol(uidVal).get()]); const remote=normalizeDB({clients:cs.docs.map(d=>d.data()).filter(Boolean),visits:vs.docs.map(d=>d.data()).filter(Boolean)}); const localClients=new Map(state.db.clients.map(c=>[c.id,c])); const localVisits=new Map(state.db.visits.map(v=>[v.id,v])); remote.clients.forEach(r=>{const l=localClients.get(r.id); if(!l||String(r.updatedAt||r.createdAt)>String(l.updatedAt||l.createdAt))localClients.set(r.id,r);}); remote.visits.forEach(r=>{const l=localVisits.get(r.id); if(!l||String(r.updatedAt||r.createdAt)>String(l.updatedAt||l.createdAt))localVisits.set(r.id,r);}); state.db=normalizeDB({clients:[...localClients.values()],visits:[...localVisits.values()]}); await idbClearAll(); await persistState();}
async function pushLocalToFirebase(){const user=fbUser(); if(!user||!requireOwner(user))throw new Error("Cuenta no autorizada."); const uidVal=user.uid; await metaRef(uidVal).set({lastPingAt:isoNow(),app:"oasis_crm_pro_v4"},{merge:true}); const ops=[]; state.db.clients.forEach(c=>ops.push({ref:clientCol(uidVal).doc(c.id),doc:c})); state.db.visits.forEach(v=>ops.push({ref:visitCol(uidVal).doc(v.id),doc:v})); for(let i=0;i<ops.length;i+=450){const batch=fbDB.batch(); ops.slice(i,i+450).forEach(item=>batch.set(item.ref,item.doc,{merge:true})); await batch.commit();}}
async function safeSyncNow(reason="manual"){if(!canAutoSync())return; if(_syncRunning){_syncPending=true;return;} _syncRunning=true;_syncPending=false; try{fbStatus(`sync ${reason}...`); await pullFirebaseToLocal(); await pushLocalToFirebase(); refreshAll(); const user=fbUser(); fbStatus(user?`online (${user.email})`:"offline");}catch(e){console.error(e);const user=fbUser();fbStatus(user?`online (${user.email})`:"offline");}finally{_syncRunning=false;if(_syncPending)setTimeout(()=>safeSyncNow("pending"),400);}}
