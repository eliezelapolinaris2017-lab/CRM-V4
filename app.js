...
function getFollowups(){
  ...
  state.db.clients.forEach(client=>{
    ...
    const visits=sortVisitsNewestFirst(state.indexes.visitsByClient.get(client.id)||[]);
    const activeVisits=visits.filter(v=>String(v.outcome||"")!=="Cancelado");
    const latestMaintenance=activeVisits.find(v=>normalizeVisitType(v.type)==="Mantenimiento") || null;
    const latestMaintenanceNext=latestMaintenance ? (isValidISODate(latestMaintenance.nextDate) ? latestMaintenance.nextDate : inferNextDate(latestMaintenance)) : "";
    const daysSinceMaintenance=latestMaintenance?.date ? daysBetween(latestMaintenance.date) : 0;
    const maintenanceDue=!!latestMaintenance && !!latestMaintenanceNext && latestMaintenanceNext < today;

    if(maintenanceDue){
      rows.push({
        kind:"due",
        priority:daysSinceMaintenance>150 ? "high" : "med",
        client,
        visit:latestMaintenance,
        lastDate:latestMaintenance.date || "—",
        reason:`Mantenimiento vencido: ${latestMaintenanceNext}`,
        action:"Mantenimiento"
      });
    }

    activeVisits
      .filter(v=>v.paymentStatus==="Pendiente"||v.paymentStatus==="Parcial"||v.outcome==="Requiere cobro")
    ...
    activeVisits
      .filter(v=>v.paymentStatus==="Cotizado"||v.outcome==="Requiere cotización"||normalizeVisitType(v.type)==="Cotización")
    ...
    activeVisits
      .filter(v=>normalizeVisitType(v.type)==="Diagnóstico" && v.followedUp!=="yes" && v.paymentStatus!=="Cotizado")
    ...
    if(client.status==="VIP" && latestMaintenanceNext && latestMaintenanceNext>=today && latestMaintenanceNext<=addDaysISO(today,14)){
      rows.push({kind:"vip",priority:"med",client,visit:latestMaintenance,lastDate:latestMaintenance?.date||stats.last||"—",reason:`Próxima: ${latestMaintenanceNext}`,action:"Coordinar"});
    }
  });
  const seen=new Set();
