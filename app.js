...
function normalizeDB(db){
  ...
  db.visits = db.visits.map(v=>({
    ...
    type: normalizeVisitType(v.type || guessType(v.service) || "Mantenimiento"),
...
function normalizeVisitType(type=""){const t=String(type||"").trim().toLowerCase(); if(t.includes("diag"))return"Diagnóstico"; if(t.includes("cot"))return"Cotización"; if(t.includes("instal"))return"Instalación"; if(t.includes("repar"))return"Reparación"; if(t.includes("cobro"))return"Cobro"; if(t.includes("mant"))return"Mantenimiento"; return String(type||"Mantenimiento").trim()||"Mantenimiento";}
function guessType(service=""){const s=String(service).toLowerCase(); if(s.includes("diagn")||s.includes("diag"))return"Diagnóstico"; if(s.includes("cot"))return"Cotización"; if(s.includes("instal"))return"Instalación"; if(s.includes("repar"))return"Reparación"; if(s.includes("cobro"))return"Cobro"; return"Mantenimiento";}
function inferNextDate(v){const type = normalizeVisitType(v.type || guessType(v.service)); if(type==="Mantenimiento") return addDaysISO(v.date || todayISO(), FOLLOWUP_DAYS); if(type==="Instalación") return addDaysISO(v.date || todayISO(), 30); return "";}
...
function getFollowups(){
  ...
  state.db.clients.forEach(client=>{
    ...
    const activeVisits=visits.filter(v=>String(v.outcome||"")!=="Cancelado");
    const maintVisits=activeVisits.filter(v=>normalizeVisitType(v.type)==="Mantenimiento");
    const noMaintenanceDays=maintVisits[0]?.date ? daysBetween(maintVisits[0].date) : 9999;
    const latestMaintenanceNext=getClientNextDateFromVisits(maintVisits);
    const maintenanceDue = client.status!=="Pausado" && maintVisits.length && ((latestMaintenanceNext && latestMaintenanceNext<=today) || (!latestMaintenanceNext && noMaintenanceDays>=FOLLOWUP_DAYS));

    if(maintenanceDue){
      const reason = latestMaintenanceNext ? `Mantenimiento vencido: ${latestMaintenanceNext}` : `${noMaintenanceDays} días sin mantenimiento`;
      rows.push({kind:"due",priority:(noMaintenanceDays>150||latestMaintenanceNext) ? "high" : "med",client,lastDate:maintVisits[0]?.date||"—",reason,action:"Mantenimiento"});
    }

    activeVisits.filter(v=>v.paymentStatus==="Pendiente"||v.paymentStatus==="Parcial"||v.outcome==="Requiere cobro")
    ...
    activeVisits.filter(v=>v.paymentStatus==="Cotizado"||v.outcome==="Requiere cotización"||normalizeVisitType(v.type)==="Cotización")
      .forEach(v=>rows.push({kind:"quote",priority:daysBetween(v.date)>7?"high":"med",client,visit:v,lastDate:v.date,reason:`${v.service}`,action:"Cotización"}));

    activeVisits.filter(v=>normalizeVisitType(v.type)==="Diagnóstico" && v.followedUp!=="yes" && v.paymentStatus!=="Cotizado")
      .forEach(v=>rows.push({kind:"diagnostic",priority:daysBetween(v.date)>3?"high":"med",client,visit:v,lastDate:v.date,reason:`Seguimiento diagnóstico · ${v.service}`,action:"Diagnóstico"}));

    if(client.status==="VIP" && latestMaintenanceNext && latestMaintenanceNext>=today && latestMaintenanceNext<=addDaysISO(today,14)){
      rows.push({kind:"vip",priority:"med",client,lastDate:maintVisits[0]?.date||stats.last||"—",reason:`Próxima: ${latestMaintenanceNext}`,action:"Coordinar"});
    }
  });
  const seen=new Set();
